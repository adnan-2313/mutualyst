...
const Landing = () => {
  ...
  return (
    <section>
      ...
      <div className="w-full max-w-[90rem] flex   text-start flex-col  pb-4 bg-[#e4e4e7] dark:bg-[#101011]  overflow-x-hidden">
        <div className="py-12  flex justify-center -mt-5   whitespace-nowrap  ">
          <h2 className="text-4xl border-b-2 border-zinc-600 sm:text-[4rem] gradient-title underline">
            Latest Stocks News
          </h2>
        </div>
        <div className="flex  flex-col  max-sm:gap-4 mb-4 -mt-10  sm:flex-row">
          <div className="w-full max-sm:px-4 sm:pl-8  ">
            <div className="w-full text-end flex   sm:justify-end">
              <h1 className="text-xl border-zinc-600 sm:text-2xl md:text-4xl sm:w-1/3 border-b-2 pr-1 text-zinc-900 dark:text-zinc-300 text-end pb-2 font-bold ">
                Top Losers
              </h1>
            </div>
            <div className="flex mt-5 sm:pr-2 flex-col w-full gap-2">
              {trendingStocks?.top_losers?.map((item) => {
                return (
                  <Card
                    className="dark:bg-zinc-800 border-none bg-zinc-100 dark:text-white border-zinc-600  w-full"
                    key={item.ticker_id}
                  >
                    <CardHeader>
          ...
          <div className="w-full max-sm:px-4 sm:pr-8   ">
            <h1 className="text-xl  md:text-4xl border-zinc-600  max-sm:ml-2 w-1/3 pl-2 pb-2 border-b-2 sm:border-l-2 text-zinc-900 dark:text-zinc-300   font-bold ">
              Top Gainers
            </h1>
            <div className="flex mt-5 flex-wrap gap-2 w-full ">
              {trendingStocks?.top_gainers?.map((item) => {
                return (
                  <Card
                    className="dark:bg-zinc-800 border-none bg-zinc-100 w-full dark:text-white dark:border-zinc-600"
                    key={item.ticker_id}
                  >
                    <CardHeader>
                      <CardTitle className=" flex justify-between  font-light">
